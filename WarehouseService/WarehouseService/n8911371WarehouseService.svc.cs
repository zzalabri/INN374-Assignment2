...
{
    // NOTE: You can use the "Rename" command on the "Refactor" menu to change the class name "n8911371WarehouseService" in code, svc and config file together.
    // NOTE: In order to launch WCF Test Client for testing this service, please select n8911371WarehouseService.svc or n8911371WarehouseService.svc.cs at the Solution Explorer and start debugging.
    public class n8911371WarehouseService : In8911371WarehouseService
    {
        ...
        public bool UpdateAmount(string componentNo, int exceptUsedAmount)
        {
            bool isUpdateSuccessful = false;// check update state
            int newAmount = 0;

            // 1. check CheckAmount only amount is enough to update
            isUpdateSuccessful = CheckAmount(componentNo, exceptUsedAmount);

            // amount is enough 
            if (isUpdateSuccessful)
            {
                // GetAmount(componentNo) can get current amount value
                newAmount = GetAmount(componentNo) - exceptUsedAmount;
